(function($){
	...
	$.fn.datagrid.defaults.operators = {
		...
		range: {
			text: '范围',
			isMatch: function(source, value){
				return source >= value.split('-')[0] && source<= value.split('-')[1];
			}
		}
	};
	...
	function addFilterRule(target, param){
		...
		if (input.length){
			...
			if (menu){
				menu.find('.'+opts.filterMenuIconCls.split(" ")[1]).removeClass(opts.filterMenuIconCls);
		...
		function initCss(){
			if (!$('#datagrid-filter-style').length){
				$('head').append(
					...
					'a.datagrid-filter-btn{display:inline-block;width:12px;height:100%;margin:0;vertical-align:middle;cursor:pointer;opacity:0.6;filter:alpha(opacity=60);}' +
